...
class Euler:
    ...
    def interface_states_explicit(self):
        ...
        for ivar in range(self.v.nvar):
            q_parabola.append(PPMInterpolant(self.grid, q[:, ivar],
                                             limit=self.use_limiting, chi_flat=None))
        ...
        for i in range(self.grid.lo-1, self.grid.hi+2):
            ...
            rho_ref = Im[i, 0, self.v.qrho]
            u_ref = Im[i, 0, self.v.qu]
            p_ref = Im[i, 0, self.v.qp]

            cs_ref = np.sqrt(self.gamma * p_ref / rho_ref)

            dum = u_ref - Im[i, 0, self.v.qu]
            dpm = p_ref - Im[i, 0, self.v.qp]

            drho = rho_ref - Im[i, 1, self.v.qrho]
            dp = p_ref - Im[i, 0, self.v.qp]

            dup = u_ref - Im[i, 2, self.v.qu]
            dpp = p_ref - Im[i, 2, self.v.qp]

            betam = 0.5 * (dpm - dum * cs_ref * rho_ref) / cs_ref**2
            if q[i, self.v.qu] - cs > 0:
                betam = 0.0

            beta0 = drho - dp / cs_ref**2
            if q[i, self.v.qu] > 0:
                beta0 = 0.0

            betap = 0.5 * (dpp + dup * cs_ref * rho_ref) / cs_ref**2
            if q[i, self.v.qu] + cs > 0:
                betap = 0.0

            q_right[i, self.v.qrho] = rho_ref - betam - betap - beta0
            ...
            u_ref = Ip[i, 2, self.v.qu]
            p_ref = Ip[i, 2, self.v.qp]

            cs_ref = np.sqrt(self.gamma * p_ref / rho_ref)

            dum = u_ref - Ip[i, 0, self.v.qu]
            dpm = p_ref - Ip[i, 0, self.v.qp]

            drho = rho_ref - Ip[i, 1, self.v.qrho]
            dp = p_ref - Ip[i, 0, self.v.qp]

            dup = u_ref - Ip[i, 2, self.v.qu]
            dpp = p_ref - Ip[i, 2, self.v.qp]
